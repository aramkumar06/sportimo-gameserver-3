...
function connect(url, socketOptions, openCallback) {
  ...
  var isSocketExternal = !!sockopts.stream;
  ...
  function onConnect() {
      ...
      if (!isSocketExternal) {
          ...
          sock.outSocket.setNoDelay(noDelay);   // connect to the outbound socksjs socket with no delay
          sock.socksSocket.setNoDelay(noDelay);   // connect to the inbound socksjs socket with no delay
      }
    ...
    c.open(fields, function(err, ok) {
      ...
      // it fouling things
        if (!isSocketExternal && timeout) sock.setTimeout(0);
      if (err === null) {
        openCallback(null, c);
      }
