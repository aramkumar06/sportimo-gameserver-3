...
module.exports = function (ModerationModule) {
    ...
    router.get('/v1/moderation/:id/service', function (req, res) {
        ...
        var result = ModerationModule.GetTournamentMatch(match_id).GetServices();
    ...
    router.get('/v1/moderation/:id/service/:league/:parserid', function (req, res) {
        ...
        ModerationModule.GetTournamentMatch(match_id).updateFeedMatchStats(req.params.league, req.params.parserid, function(err, result){
        if(err)
    ...
    router.post('/v1/moderation/:id/service/add', function (req, res) {
        var match_id = req.params.id;
        ModerationModule.GetTournamentMatch(match_id).AddModerationService(req.body, function(error, result) {
            if (error != null)
    ...
    router.put('/v1/moderation/:id/service/pause', function (req, res) {
        var match_id = req.params.id;
        ModerationModule.GetTournamentMatch(match_id).PauseService(req.body, function(error, result) {
            if (error != null)
                return res.status(500).json({ error: error.message });
    ...
    router.put('/v1/moderation/:id/service/resume', function (req, res) {
        var match_id = req.params.id;
        ModerationModule.GetTournamentMatch(match_id).ResumeService(req.body, function(error, result) {
            if (error != null)
